...
int llama_wrapper_model_n_cls_out(void* model) {
    if (!model) {
        return -1;
    }
    auto model_wrapper = static_cast<llama_wrapper_model_t*>(model);
    return (int)llama_model_n_cls_out(model_wrapper->model);
}

const char* llama_wrapper_model_cls_label(void* model, int index) {
    if (!model || index < 0) {
        return nullptr;
    }
    auto model_wrapper = static_cast<llama_wrapper_model_t*>(model);
    return llama_model_cls_label(model_wrapper->model, (uint32_t)index);
}

int llama_wrapper_device_count() {
    return (int)ggml_backend_dev_count();
}

bool llama_wrapper_device_get(int index, llama_wrapper_device_info* info) {
    if (!info || index < 0 || (size_t)index >= ggml_backend_dev_count()) {
        return false;
    }
    ...
    snprintf(info->name, sizeof(info->name), "%s", ggml_backend_dev_name(dev));
    snprintf(info->description, sizeof(info->description), "%s", ggml_backend_dev_description(dev));
    info->type = (int)ggml_backend_dev_type(dev);
    size_t free_mem = 0, total_mem = 0;
    ggml_backend_dev_memory(dev, &free_mem, &total_mem);
    info->free_memory_mb = (int)(free_mem / (1024 * 1024));
    info->total_memory_mb = (int)(total_mem / (1024 * 1024));
    return true;
}

bool llama_wrapper_supports_gpu_offload() {
...
// Rank/relevance score: mirrors llama_wrapper_embeddings almost exactly (same
// tokenize -> decode -> llama_get_embeddings_seq flow), but sized by
// n_cls_out (a classifier's output count, typically 2 for Qwen3-Reranker's
// yes/no head) instead of n_embd (the model's hidden size) — llama.cpp
// applies softmax over the classifier logits directly in the graph for
// Qwen3-arch rerankers, so what comes back here is already a normalised
// probability per class, no further math needed on the caller's side.
int llama_wrapper_rank_score(void* ctx, const char* text, float* out_scores, int max_scores) {
    if (!ctx || !text || !out_scores) {
        g_last_error = "Invalid parameters for rank score";
        return -1;
    }
    ...
    try {
        ...
        std::vector<llama_token> tokens = common_tokenize(wrapper->ctx, text, true, true);
        if (tokens.empty()) {
            g_last_error = "Failed to tokenize text for rank score";
            return -1;
        }
        ...
        for (int i = 0; i < n_tokens; i += n_batch) {
            ...
            if (llama_decode(wrapper->ctx, batch) != 0) {
                llama_batch_free(batch);
                g_last_error = "Failed to decode tokens for rank score";
                return -1;
            }
        ...
        const float* embd = llama_get_embeddings_seq(wrapper->ctx, 0);
        if (!embd) {
            g_last_error = "Failed to get rank score from context (does this model have pooling_type == RANK? a naively-converted reranker GGUF often lacks the classifier head)";
            return -1;
        }

        int n_cls_out = (int)llama_model_n_cls_out(wrapper->model);
        int count = std::min(n_cls_out, max_scores);
        memcpy(out_scores, embd, count * sizeof(float));

        return count;
    } catch (const std::exception& e) {
        g_last_error = "Exception during rank score generation: " + std::string(e.what());
        return -1;
    }
}
