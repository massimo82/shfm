...
// Classifier/reranker introspection (only meaningful for a model whose GGUF
// declares a classifier head, e.g. a properly-converted Qwen3-Reranker).
int llama_wrapper_model_n_cls_out(void* model);
const char* llama_wrapper_model_cls_label(void* model, int index);

// Compute backends actually available to this build, via ggml's generic
// device registry (unlike llama_wrapper_get_gpu_info, which is CUDA-only
// and always reports nothing for Vulkan/SYCL/OpenCL/Metal/... builds).
// type: 0=CPU, 1=GPU (discrete), 2=IGPU (integrated), 3=ACCEL, 4=META.
typedef struct {
    char name[64];
...
// Rank/relevance score for text (a fully pre-formatted query+document
// prompt) against a context whose model has pooling_type == RANK. Writes up
// to max_scores floats (see llama_wrapper_model_n_cls_out for the real
// count) into out_scores. Returns the number of floats written, or -1 on
// error (including when the loaded model has no classifier head at all).
