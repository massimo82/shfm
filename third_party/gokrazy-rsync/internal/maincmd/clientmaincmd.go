...
func ClientRun(osenv *rsyncos.Env, opts *rsyncopts.Options, conn io.ReadWriteCloser, paths []string, negotiate bool) (*rsyncstats.TransferStats, error) {
	...
	if opts.Sender() {
		...
		// shfm: a receiver running with --delete reads the filter list
		// before the file list (rsync/exclude.c:send_filter_list sends it
		// whenever the receiver wants it), so send it — otherwise both
		// sides block waiting for each other.
		if opts.DeleteMode() {
