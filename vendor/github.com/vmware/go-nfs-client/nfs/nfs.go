...
func dialService(addr string, port int) (*rpc.Client, error) {
	...
	// Many NFS servers reject MOUNT/NFS calls that don't originate from a
	// "reserved" port (<1024) — the "secure" export option, which is the
	// default. Try one first regardless of whether we're root: a non-root
	// process can still bind a reserved port if granted the
	// CAP_NET_BIND_SERVICE capability (e.g. `setcap cap_net_bind_service=+ep`
	// on the binary), exactly how libnfs — which is what GVfs/Thunar's own
	// NFS backend is built on — documents doing the same thing. If we don't
	// have that privilege, binding fails with EACCES and we fall back to a
	// normal ephemeral port (which only works if the server's export has
	// the "insecure" option).
	...
	return rpc.DialTCP("tcp", nil, raddr)
}
